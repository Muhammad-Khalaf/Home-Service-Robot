...
int main(int argc, char** argv) {
	...
	goal.target_pose.pose.position.y = 2.9;
	...
	{
		ROS_INFO("SUCESS: pick up location");
		...
		if (ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED)
		{
			ROS_INFO("SUCESS: drop off location");
		}
		else
			ROS_INFO("FAILED: drop off location");
	}
	else
		ROS_INFO("FAILED: pick up location");
